document.addEventListener('DOMContentLoaded', () => {
    ...
    const userParentNameInput = document.getElementById('userParentNameInput');
    const userParentNameWrap = document.getElementById('userParentNameWrap');
    const parentMatchStatus = document.getElementById('parentMatchStatus');
    const createUserForm = document.getElementById('createUserForm');
    const createUserPassword = document.getElementById('createUserPassword');

    // Program (4Ps / ARAL), Section, and Parent's Name only apply to students
    function syncProgramVisibility() {
        ...
        userParentNameWrap.style.display = isStudent ? '' : 'none';
        if (!isStudent) {
            userProgramSelect.value = 'none';
            userSectionSelect.value = '';
            userParentNameInput.value = '';
            parentMatchStatus.textContent = '';
        }
    }
    ...
    createUserForm.addEventListener('submit', async (e) => {
        ...
        if (userRoleSelect.value === 'Student' && userParentNameInput.value.trim()) {
            payload.parentName = userParentNameInput.value.trim();
        }
        ...
        if (data.parentLinkMessage) message += `\n\n${data.parentLinkMessage}`;
        alert(message);
    ...
    // --- Parent-Student Links (read-only table; links are created automatically
    // during student creation via the Parent's Full Name field) ---
    ...
    // Autocomplete suggestions for the "Parent's Full Name" field on the create form
    async function loadParentNameSuggestions() {
        const data = await authedFetch('/api/users?role=Parent&limit=1000', token);
        const datalist = document.getElementById('parentNameSuggestions');
        if (data.success && datalist) {
            datalist.innerHTML = data.users.map(p => `<option value="${p.first_name} ${p.last_name}"></option>`).join('');
        }
    }
    loadParentNameSuggestions();
